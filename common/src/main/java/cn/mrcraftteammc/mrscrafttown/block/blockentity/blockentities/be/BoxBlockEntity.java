...
public class BoxBlockEntity extends RandomizableContainerBlockEntity {
    ...
    public BoxBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(MrCTBlockEntitiesTypes.BOX_BLOCK_ENTITY_TYPE.get(), blockPos, blockState);
    }
    ...
    public void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), (BlockState)state.setValue(BoxBlocks.OPEN, open), 3);
    }

    public void playSound(BlockState state, SoundEvent sound) {
        Vec3i vec3i = state.getValue(BoxBlocks.FACING).getNormal();
