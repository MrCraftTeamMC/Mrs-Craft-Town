...
public class LargeBoxBlockEntity extends RandomizableContainerBlockEntity {
    ...
    private final ContainerOpenersCounter openersCounter = new ContainerOpenersCounter(){
        ...
        protected boolean isOwnContainer(Player player) {
            if (player.containerMenu instanceof ChestMenu) {
                ...
                return container == LargeBoxBlockEntity.this;
            }
            return false;
        }
    };

    public LargeBoxBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(MrCTBlockEntitiesTypes.LARGE_BOX_BLOCK_ENTITY_TYPE.get(), blockPos, blockState);
    }
    ...
    public void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), (BlockState)state.setValue(LargeBoxBlocks.OPEN, open), 3);
    }

    public void playSound(BlockState state, SoundEvent sound) {
        Vec3i vec3i = state.getValue(LargeBoxBlocks.FACING).getNormal();
